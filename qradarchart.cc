...
QRadarChart::QRadarChart(QWidget *parent): QWidget(parent) {}

void QRadarChart::addProperty(QString propertyName, int propertyValue) {
  m_properties.insert(propertyName, propertyValue);
  update();
}

void QRadarChart::paintEvent(QPaintEvent *event) {
  Q_UNUSED(event);
